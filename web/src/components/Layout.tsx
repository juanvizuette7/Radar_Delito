...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div
      className={[
        "relative min-h-screen overflow-x-hidden bg-slate-50 text-slate-900 transition-colors duration-300 dark:bg-ink dark:text-white",
        isDashboardRoute ? "xl:h-[100dvh] xl:overflow-hidden" : "",
      ].join(" ")}
    >
      ...
      <main
        className={[
          ...
          isDashboardRoute
            ? "pb-5 pt-28 sm:pt-32 xl:h-[100dvh] xl:overflow-hidden"
            : "pb-12 pt-32 sm:pt-36",
        ].join(" ")}
      >
        <div className={["mx-auto max-w-8xl", isDashboardRoute ? "xl:h-full" : ""].join(" ")}>
          {children}
        </div>
      </main>
